...
const FormationsSingle = (props) => {
    console.log(useLocation())
    const [course, setCourse] = useState(null)
    const {state: {id}} = useLocation()
    useEffect(() => {
        const fetchCourse = async () => {
            const resp = await fetch(`http://localhost:3002/formation/${id}`);
            const respJson = await resp.json()
            console.log(respJson)
            setCourse(respJson.formation)
        }
        if(id){
            fetchCourse()
        }
        ...
        <div class="main-content">
            <div class="sc-breadcrumbs breadcrumbs-overlay">
                ...
                <div class="breadcrumbs-text white-color">
                    <h1 class="page-title" style={{color: "#000", paddingTop: "200px"}}>{course && course?.name}</h1>
                  
                </div>
            </div>
            <section class="sc-course-details pt-120 pb-70  md-pb-40">
                <div class="container">
                                ...
                                <div class="course-features-info">
                                    ...
                                    <ul>
                                        ...
                                        <i class="fa fa-puzzle-piece"></i>
                                            <span class="value">{course && course.name} </span>
                                        </li>
                                       
                                        <li class="quizzes-feature">
                                            <i class="fa fa-puzzle-piece"></i>
                                            <span class="label">Departement</span>
                                            <span class="value">{course && course.departement.name}</span>
                                        </li>
                                      
                                        <li class="students-feature">
                                            <i class="fa fa-users"></i>
                                            <span class="label">Etablissement</span>
                                            <span class="value">{course ? course.departement?.etablissement?.name : 0}</span>
                                        </li>
